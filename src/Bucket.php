...
//        return Vaults::get($name, $arguments);
    ...
    private function getAllConfigs(): void
    {
        ...
        if (is_readable($cacheConfigPath)) {
            ...
            $config = (require $configPath)['bucket'];
        } else {
            $configPath = dirname(getcwd()) . DIRECTORY_SEPARATOR . 'config' . DIRECTORY_SEPARATOR . 'bucket.php';

            if (!is_readable($configPath)) {
                throw new InvalidConfigException('配置文件不存在或不可读[配置文件应当存在于项目根目录下的`config`文件夹, 并命名为`bucket.php`]', Exception::CONFIG_FILE_ERROR);
            }
